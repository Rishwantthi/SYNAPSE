...
st.set_page_config(page_title="Farmer Planner App", page_icon="🌱")
...
if st.button("Bio digital twin"):
    js = f"window.open('https://79ae9c88-0d88-46e5-8a4f-38575f2c6f88-00-2fndls089eiyh.spock.replit.dev/', '_blank')"
    st.components.v1.html(f"<script>{js}</script>", height=0)

if st.button("Voice integreted CHATBOT"):
    js = f"window.open('https://283655ee2ad88cf9c5.gradio.live', '_blank')"
    st.components.v1.html(f"<script>{js}</script>", height=0)

if st.button("PRICE TREND PREDICTOR"):
    js = f"window.open('https://6733bf8387990a867a.gradio.live', '_blank')"
    st.components.v1.html(f"<script>{js}</script>", height=0)

if st.button("Soil health Predictor"):
    js = f"window.open('https://373f6a926f7dd7e747.gradio.live', '_blank')"
    st.components.v1.html(f"<script>{js}</script>", height=0)
```
